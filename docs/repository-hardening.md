...
- Generated declarations ship with the package and are checked with a strict NodeNext consumer, including portable GSAP update/transition definitions and expected type failures. The optional `route-graphics/module` entry shares Pixi, GSAP, and keyboard dependencies with a host bundle. Audio decoders remain bundled so their unused optional Opus ML model stays excluded.
...
- Full unit/component/CLI suite: 1,794 tests passed across 124 files, including persistent update reconciliation and point-interval overlap regressions.
